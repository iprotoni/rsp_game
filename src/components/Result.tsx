...
const Result:FC<TResult> = ({status, received, contractChoice, hash, userChoice}) => {
  return(
    <div className="mt-8 flex flex-col text-center self-center top-[50%] p-6">
      ...
      {received ? <p className="text-xl font-semibold mt-2 ">Received: {received} tBNB</p> : null}
      <a className="mt-8 underline underline-offset-1" target="_blank" rel="noreferrer" href={`https://testnet.bscscan.com/tx/${hash}`}>BscScan transaction</a>
    </div>
  )
};
